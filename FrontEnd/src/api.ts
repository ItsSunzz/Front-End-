...
export const api = {
  ...
  async updateRecipient(id: number, fields: any) {
    // Check if fields or nested fields contain File objects
    let hasFiles = false;
    ...
    if (hasFiles) {
      ...
      Object.entries(fields).forEach(([key, val]) => {
        if (key === 'newFiles' && val) {
          Object.entries(val).forEach(([fileKey, fileVal]) => {
            if (fileVal instanceof File) {
              body.append(fileKey, fileVal);
            }
          });
        } else if (key !== 'dokumenUploaded' && val !== null && val !== undefined) {
  ...
  async renewRecipient(id: number, reuseDocuments: boolean, files?: Record<string, File>) {
    const body = new FormData();
    body.append('reuse_documents', String(reuseDocuments));
    if (files) {
    ...
    return apiFetch(`/pengajuan/${id}/renew`, {
      method: 'POST',
      body,
    });
  },
