...
export default function App() {
  ...
  const triggerToast = (message: string, type: 'success' | 'error' | 'warning' = 'success', duration = 4000) => {
  ...
  // Poll recipients list every 10 seconds for real-time updates when logged in
  React.useEffect(() => {
  ...
  // Detect new rejections (Ditolak) for Kecamatan Admin to alert them to resubmit
  const [notifiedDitolakIds, setNotifiedDitolakIds] = React.useState<string[]>([]);
  const [isFirstFetchDone, setIsFirstFetchDone] = React.useState(false);

  React.useEffect(() => {
    if (!userSession.isLoggedIn || userSession.role !== 'Admin Kecamatan') {
      if (isFirstFetchDone) setIsFirstFetchDone(false);
      if (notifiedDitolakIds.length > 0) setNotifiedDitolakIds([]);
      return;
    }

    if (recipients.length === 0) return;

    const ditolakRecipients = recipients.filter(
      r => r.status === 'Ditolak' && r.kecamatan === userSession.kecamatan
    );
    const currentDitolakIds = ditolakRecipients.map(r => r.id?.toString() || r.nik);

    if (!isFirstFetchDone) {
      if (currentDitolakIds.length > 0) {
        triggerToast(`⚠️ Terdapat ${currentDitolakIds.length} pengajuan yang ditolak. Mohon segera lakukan pengajuan ulang!`, 'warning', 5000);
        playNotificationSound();
      }
      setNotifiedDitolakIds(currentDitolakIds);
      setIsFirstFetchDone(true);
      return;
    }

    const newDitolakRecipients = ditolakRecipients.filter(
      r => !notifiedDitolakIds.includes(r.id?.toString() || r.nik)
    );

    if (newDitolakRecipients.length > 0) {
      const names = newDitolakRecipients.map(r => r.nama).join(', ');
      triggerToast(`⚠️ Pengajuan ditolak (${names}). Mohon segera lakukan pengajuan ulang!`, 'warning', 5000);
      playNotificationSound();
    }

    // Keep the notified list in sync with current Ditolak IDs so resolved ones are removed
    setNotifiedDitolakIds(currentDitolakIds);
  }, [recipients, userSession.isLoggedIn, userSession.role, userSession.kecamatan, isFirstFetchDone, notifiedDitolakIds]);
